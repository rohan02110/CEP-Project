...
class VehicleDamageAssessmentPipeline:
    ...
    def __init__(
        ...
        # 6. Load Glass Integrity & Micro-Fracture Analyzer
    ...
    def validate_glass_shatter(
        ...
        """
        Evaluates micro-fractures, edge gradient entropy, Laplacian variance,
        and specular glare masks to verify whether a window is truly shattered/cracked or intact.
        """
        img_h, img_w = cv_img_bgr.shape[:2]
        x1, y1, x2, y2 = [int(max(0, min(v, img_w if i % 2 == 0 else img_h))) for i, v in enumerate(bbox_xyxy)]
        crop = cv_img_bgr[y1:y2, x1:x2]
        return self.glass_analyzer.analyze_window_crop(crop, detector_conf=conf, sensitivity=sensitivity)
    ...
    def assess_image(
        ...
        default_class_confs = {
            ...
            "glass shatter": max(0.22, conf_threshold),
        ...
        if len(det_results) > 0 and det_results[0].boxes is not None:
            ...
            for cls_name, conf, xyxy in raw_candidates:
                ...
                # 4. Glass Shatter / Intact Window Multi-Cue Verification Filter
                if cls_name == "glass shatter" and validate_glass:
                    diag = self.validate_glass_shatter(cv_img_bgr, xyxy, conf, sensitivity=glass_sensitivity)
...
def test_pipeline_on_sample_images():
    ...
    for idx, (img_p, veh) in enumerate(zip(test_images, vehicles)):
        ...
        print(f"  -> Filtered BG Damages: {result.filtered_background_damages_count}, Suppressed Tires: {result.suppressed_tire_false_positives_count}, Suppressed Glass: {result.suppressed_glass_false_positives_count}")
